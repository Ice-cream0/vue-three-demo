...
let scene, hdrCubeRenderTarget, building, arrpoint, arrline, park, group, groups, groupone, title;
export default {
  name: "ModelThree",
  data() {
    return {
  ...
  methods: {
    ...
    title(name, position) {
      ...
      let sprite = new THREE.Sprite(spriteMaterial);
      sprite.scale.set(c.width / 1.5, c.height / 1.5, 0);
      sprite.position.set(position.x, 1300, position.z);
      title.add(sprite);
      scene.add(title);
    },
    ...
    init() {
      ...
      arrpoint = new THREE.Group();
      arrline = new THREE.Group();
    ...
    change(index) {
      ...
      this.clearGroup(arrline);
      this.clearGroup(arrpoint);
      this.clearGroup(title);
      this.title("xxxxxxxxxx公司", this.newPosition);
      this.count1 = index;
        ...
        if (child instanceof Object && child.name == item) {
          ...
          tween.onComplete(() => {
            ...
            if (index >= 8) {
              ...
              this.title("xxxxxxxxxx公司", child.position);
            }
          })
          tween.start()
          tween1.start()
